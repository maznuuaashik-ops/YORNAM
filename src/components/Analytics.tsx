    ...
    try {
      ...
      if (data) {
        ...
        if (data.length > 0) {
          setStats({
            total: data.length,
            avgScore: Math.round(data.reduce((sum, a) => sum + a.overall_score, 0) / data.length),
            bestScore: Math.max(...data.map(a => a.overall_score)),
            avgHook: Math.round(data.reduce((sum, a) => sum + a.hook_strength, 0) / data.length),
          })
        }
      }
    } catch (err) {
