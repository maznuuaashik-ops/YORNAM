...
function CircularProgress({ score, size = 200 }: { score: number; size?: number }) {
  ...
  const getScoreConfig = (s: number) => {
    if (s >= 80) return { color: '#10b981', label: 'Excellent', glow: 'rgba(16, 185, 129, 0.35)' }
    if (s >= 60) return { color: '#3b82f6', label: 'Good', glow: 'rgba(59, 130, 246, 0.35)' }
    if (s >= 40) return { color: '#f59e0b', label: 'Average', glow: 'rgba(245, 158, 11, 0.35)' }
...
export default function ViralScore({ score, subMetrics, enhancedReport }: ViralScoreProps) {
  ...
  const getVerdict = (s: number) => {
    if (s >= 85) return { text: 'Viral Potential', icon: Flame, color: '#10b981' }
    if (s >= 65) return { text: 'Strong Performer', icon: TrendingUp, color: '#3b82f6' }
    return { text: 'Needs Improvement', icon: Eye, color: '#f59e0b' }
  }
  ...
  const viralProbConfig = useMemo(() => {
    ...
    if (v >= 70) return { color: '#10b981', label: 'High', glow: 'rgba(16, 185, 129, 0.3)' }
    if (v >= 45) return { color: '#3b82f6', label: 'Moderate', glow: 'rgba(59, 130, 246, 0.3)' }
    return { color: '#f59e0b', label: 'Low', glow: 'rgba(245, 158, 11, 0.3)' }
  }, [enhancedReport.viralProbability])

  const { watchTimeEstimate, engagementPrediction } = enhancedReport
  const typedVerdict = useTypewriter(enhancedReport.finalVerdict, inView)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-sm sm:text-base max-w-lg mx-auto" style={{ color: 'var(--text-tertiary)' }}>
            AI-computed virality potential based on 47 engagement signals across 5 dimensions
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Viral Probability</h3>
            </div>
            ...
            <p className="text-xs mt-3" style={{ color: 'var(--text-tertiary)' }}>
              Chance of exceeding 100K organic impressions within 48 hours
            </p>
            {/* Confidence level */}
            <div className="mt-4 pt-3" style={{ borderTop: '1px solid var(--divider)' }}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[10px] tracking-wider uppercase" style={{ color: 'var(--text-quaternary)' }}>Confidence</span>
                <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{
                  ...
                  {enhancedReport.confidence.score}% {enhancedReport.confidence.level}
                </span>
              </div>
              ...
              <p className="text-[11px] mt-2 leading-relaxed" style={{ color: 'var(--text-quaternary)' }}>
                {enhancedReport.confidence.reasoning}
              </p>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Watch Time Estimate</h3>
            </div>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div>
                <div className="text-xl font-bold tabular-nums" style={{ color: 'var(--text-primary)' }}>{watchTimeEstimate.avgWatchTime}</div>
                <div className="text-[10px] tracking-wider uppercase" style={{ color: 'var(--text-quaternary)' }}>Avg Watch</div>
              </div>
              <div>
                <div className="text-xl font-bold tabular-nums" style={{ color: 'var(--text-primary)' }}>{watchTimeEstimate.completionRate}%</div>
                <div className="text-[10px] tracking-wider uppercase" style={{ color: 'var(--text-quaternary)' }}>Completion</div>
              </div>
              <div>
                ...
                <div className="text-[10px] tracking-wider uppercase" style={{ color: 'var(--text-quaternary)' }}>Replays</div>
              </div>
            </div>
            <div className="text-xs mb-2 font-medium" style={{ color: 'var(--text-tertiary)' }}>Retention Curve</div>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Engagement Prediction</h3>
            </div>
            <div className="space-y-2.5">
              <EngagementStat icon={Heart} label="Est. Likes" value={engagementPrediction.estimatedLikes} color="#f43f5e" delay={0.6} />
              <EngagementStat icon={MessageCircle} label="Est. Comments" value={engagementPrediction.estimatedComments} color="#3b82f6" delay={0.65} />
              <EngagementStat icon={Share2} label="Est. Shares" value={engagementPrediction.estimatedShares} color="#06b6d4" delay={0.7} />
              <EngagementStat icon={Bookmark} label="Est. Saves" value={engagementPrediction.estimatedSaves} color="#818cf8" delay={0.75} />
            </div>
            <div className="mt-4 pt-3 flex items-center justify-between" style={{ borderTop: '1px solid var(--divider)' }}>
              <div>
                <div className="text-xs" style={{ color: 'var(--text-quaternary)' }}>Engagement Rate</div>
                <div className="text-lg font-bold" style={{ color: 'var(--accent-cyan)' }}>{engagementPrediction.engagementRate}%</div>
              </div>
              <div className="text-right">
                <div className="text-xs" style={{ color: 'var(--text-quaternary)' }}>Virality Multiplier</div>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>Final AI Verdict</h3>
