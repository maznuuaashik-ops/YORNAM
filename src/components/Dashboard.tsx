...
function getStatus(score: number): { label: string; color: string } {
  if (score >= 85) return { label: 'Viral', color: '#10b981' }
  if (score >= 65) return { label: 'Strong', color: '#3b82f6' }
  return { label: 'Average', color: '#f59e0b' }
}

function buildMetricCards(m: DashboardMetrics) {
  return [
    { icon: TrendingUp, label: 'Projected Views', value: m.projectedViews, sub: m.projectedViewsSub, color: '#3b82f6' },
    { icon: Share2, label: 'Share Probability', value: m.shareProbability, sub: m.shareProbSub, color: '#06b6d4' },
    { icon: Clock, label: 'Best Post Time', value: m.bestPostTime, sub: m.bestPostTimeSub, color: '#6366f1' },
    { icon: Video, label: 'Ideal Duration', value: m.idealDuration, sub: m.idealDurationSub, color: '#60a5fa' },
    { icon: Target, label: 'Target Audience', value: m.targetAudience, sub: m.targetAudienceSub, color: '#38bdf8' },
    { icon: Award, label: 'Content Rank', value: m.contentRank, sub: m.contentRankSub, color: '#f59e0b' },
  ]
}
    ...
    >
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Audience Match</h3>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div>
          <div className="text-[10px] tracking-wider uppercase mb-1" style={{ color: 'var(--text-quaternary)' }}>Primary Age</div>
          <div className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>{audience.primaryAgeRange}</div>
        </div>
        <div>
          <div className="text-[10px] tracking-wider uppercase mb-1" style={{ color: 'var(--text-quaternary)' }}>Secondary Age</div>
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] tracking-wider uppercase" style={{ color: 'var(--text-quaternary)' }}>Gender Split</span>
        </div>
        ...
        <div className="flex justify-between mt-1.5">
          <span className="text-xs" style={{ color: '#3b82f6' }}>Male {audience.genderSplit.male}%</span>
          <span className="text-xs" style={{ color: '#f43f5e' }}>Female {audience.genderSplit.female}%</span>
        </div>
      </div>

      {/* Top interests */}
      <div className="mb-4">
        <div className="text-[10px] tracking-wider uppercase mb-2" style={{ color: 'var(--text-quaternary)' }}>Top Interests</div>
...
function CaptionQualityCard({ score, inView }: { score: number; inView: boolean }) {
  const tier = score >= 75 ? { color: '#10b981', label: 'Strong' } : score >= 50 ? { color: '#3b82f6', label: 'Moderate' } : { color: '#f59e0b', label: 'Weak' }
    ...
    >
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Caption Quality</h3>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Content Category</h3>
      </div>
      <div className="text-2xl font-bold mb-1" style={{ color: 'var(--accent-cyan)' }}>{category}</div>
      <p className="text-xs" style={{ color: 'var(--text-quaternary)' }}>AI-detected primary content niche</p>
    </motion.div>
  )
}
...
export default function Dashboard({ metrics, recentAnalyses, enhancedReport }: DashboardProps) {
  ...
  const metricCards = metrics ? buildMetricCards(metrics) : null
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-sm sm:text-base" style={{ color: 'var(--text-tertiary)' }}>
            Your personalized analytics command center
          </p>
        </motion.div>
        ...
        {!metricCards && (
          <div className="glass-card rounded-2xl p-8 sm:p-12 text-center mb-8">
            ...
            <h3 className="text-base font-semibold mb-1" style={{ color: 'var(--text-secondary)' }}>No Analysis Yet</h3>
            <p className="text-sm" style={{ color: 'var(--text-quaternary)' }}>Analyze your first reel to see dashboard metrics</p>
          </div>
        )}
        ...
        {recentAnalyses.length > 0 && (
          ...
          >
            <div className="px-5 sm:px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--divider)' }}>
              <h3 className="font-semibold text-sm sm:text-base" style={{ color: 'var(--text-primary)' }}>Recent Reel History</h3>
            ...
            <div>
              {recentAnalyses.map((reel, i) => {
                const status = getStatus(reel.overall_score)
                return (
                  <motion.div
