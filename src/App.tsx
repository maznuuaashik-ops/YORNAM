...
import { Zap, Menu, X, LogOut, User, BarChart3, Settings as SettingsIcon, Shield, Megaphone, XCircle, Info, Mail, Lock } from 'lucide-react'
import { analyzeReel } from './lib/api'
        ...
        <div className="hidden lg:flex items-center gap-1 text-sm" style={{ color: 'var(--text-tertiary)' }}>
          ...
          {isAdmin && (
            <Link
              ...
              className="relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all duration-300 group"
              style={{
                color: location.pathname === '/admin' ? 'var(--accent-blue)' : 'var(--text-tertiary)',
                background: location.pathname === '/admin' ? 'rgba(59,130,246,0.08)' : 'transparent',
              }}
            >
              <Shield size={14} className="transition-transform duration-300 group-hover:scale-110" style={{ color: location.pathname === '/admin' ? 'var(--accent-blue)' : undefined }} />
              <span className="transition-colors duration-300">Admin</span>
            </Link>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          {user ? (
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer"
            ...
            <div className="flex flex-col gap-2">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.to}
                    to={link.to}
                    className="flex items-center gap-2 text-sm py-2 px-3 rounded-xl transition-all duration-300"
                    style={{
              ...
              {isAdmin && (
                <Link
                  ...
                  className="flex items-center gap-2 text-sm py-2 px-3 rounded-xl transition-all duration-300"
                  style={{
                    color: location.pathname === '/admin' ? 'var(--accent-blue)' : 'var(--text-secondary)',
                    background: location.pathname === '/admin' ? 'rgba(59,130,246,0.08)' : 'var(--bg-subtle)',
                    border: location.pathname === '/admin' ? '1px solid rgba(59,130,246,0.2)' : '1px solid transparent',
                  }}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <Shield size={14} />
              ...
              {user ? (
                <button
                  ...
                  className="flex items-center gap-2 text-sm py-2.5 px-3 rounded-xl cursor-pointer"
...
function HomePage() {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <Hero onAnalyze={handleAnalyze} isLoading={isLoading} error={error} />
